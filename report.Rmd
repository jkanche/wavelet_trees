...
```{r}
times <- rjson::fromJSON(file="wt/task1-times.json")
...
sizes <- rjson::fromJSON(file="wt/task1-size.json")
names(sizes) <- xlabels
...
```{r}
times <- rjson::fromJSON(file="wt/task2-times.json")
xlabels <- paste0("2^", seq(from=10, to=20))
names(times) <- xlabels
times <- as.data.frame(times)
colnames(times) <- xlabels
...
sizes <- rjson::fromJSON(file="wt/task2-size.json")
names(sizes) <- xlabels
...
```{r}
times <- rjson::fromJSON(file="wt/task3.1-times.json")
...
times <- rjson::fromJSON(file="wt/task3.2-times.json")
times <- times[-c(1,2,3)]
xlabels <- seq(4, 8)
names(times) <- xlabels
times <- as.data.frame(times)
